/* ============================================
   STOCKR — adminDashboard.js
   Rewritten to match actual Spring Boot backend
...
function loadUsers() {
    ...
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:rgba(255,255,255,.4);padding:3rem;">Loading users...</td></tr>';
    fetch(API_BASE + '/users', { credentials:'include' })
        ...
        .catch(function() {
            allUsers = [
                { id:1, name:'Admin',   email:'admin@stockr.com',   role:'ADMIN',   isActive:true  },
                { id:2, name:'Lahiru',  email:'lahiru@stockr.com',  role:'MANAGER', isActive:true  },
                { id:3, name:'Hasitha', email:'hasitha@stockr.com', role:'STOCKER', isActive:true  },
                { id:4, name:'Test',    email:'test@stockr.com',    role:'STOCKER', isActive:false }
            ];
            renderUserTable(allUsers);
            showToast('[DEMO] Using local user data', 'fail');
        });
}

function renderUserTable(users) {
    document.getElementById('stat-total-users').textContent     = users.length;
    document.getElementById('stat-active-users').textContent    = users.filter(function(u){return u.isActive;}).length;
    document.getElementById('stat-suspended-users').textContent = users.filter(function(u){return !u.isActive;}).length;
    document.getElementById('stat-admin-count').textContent     = users.filter(function(u){return u.role==='ADMIN';}).length;

    var tbody = document.getElementById('user-table-body');
    tbody.innerHTML = '';
    if (!users.length) { tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:3rem;color:rgba(255,255,255,.4);">No users found.</td></tr>'; return; }
    users.forEach(function(u) {
        var tr = document.createElement('tr');
        tr.innerHTML = '<td>#' + u.id + '</td>' +
            '<td style="color:#fff;">' + escHtml(u.name) + '</td>' +
            '<td>' + escHtml(u.email) + '</td>' +
            '<td><span class="badge-status ' + (u.isActive?'active':'suspended') + '" onclick="toggleUserStatus(' + u.id + ')" title="Click to toggle">' + (u.isActive?'ACTIVE':'SUSPENDED') + '</span></td>' +
...
function filterUsers(q) {
    ...
    renderUserTable(allUsers.filter(function(u) {
        var matchQ = !query || u.name.toLowerCase().indexOf(query) !== -1 || u.email.toLowerCase().indexOf(query) !== -1;
        var matchR = !roleFilter || u.role === roleFilter;
        return matchQ && matchR;
    }));
}

var userModalMode = 'add', editingUserId = null;

function openUserModal(mode, userId) {
    userModalMode = mode; editingUserId = userId || null;
    clearErrs(['modal-name','modal-email','modal-password'], ['modal-name-err','modal-email-err','modal-pw-err']);
    document.getElementById('modal-pw-wrap').style.display = mode === 'add' ? 'flex' : 'none';
    if (mode === 'add') {
        document.getElementById('user-modal-title').textContent = 'Add User';
        document.getElementById('user-modal-sub').textContent   = '// CREATE A NEW SYSTEM USER';
        document.getElementById('modal-name').value = '';
        document.getElementById('modal-email').value = '';
        document.getElementById('modal-password').value = '';
        document.getElementById('modal-role').value = 'STOCKER';
    } else {
        var u = allUsers.find(function(x){return x.id === userId;}); if (!u) return;
        document.getElementById('user-modal-title').textContent = 'Edit User';
        document.getElementById('user-modal-sub').textContent   = '// UPDATE USER DETAILS';
...
function saveUser() {
    ...
    var valid = true;
    if (name.length < 2) { markErr('modal-name','modal-name-err'); valid = false; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { markErr('modal-email','modal-email-err'); valid = false; }
    if (userModalMode === 'add' && password.length < 8) { markErr('modal-password','modal-pw-err'); valid = false; }
    if (!valid) return;

    var btn = document.getElementById('modal-save-btn');
    setBtnLoading(btn, true);
    var url = userModalMode === 'add' ? API_BASE + '/users' : API_BASE + '/users/' + editingUserId;
    var method = userModalMode === 'add' ? 'POST' : 'PUT';
    // Backend PUT only accepts {name, role} — email cannot be updated
    var body = userModalMode === 'add' ? {name:name, email:email, password:password, role:role} : {name:name, role:role};

    fetch(url, { method:method, credentials:'include', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body) })
        .then(function(r){return r.json();})
        .then(function(data){
            if (data.success) { showToast(data.message,'success'); closeUserModal(); loadUsers(); }
            else showToast(data.message || 'Failed.','fail');
            setBtnLoading(btn, false);
        })
        .catch(function(){
            if (userModalMode === 'add') {
                allUsers.push({ id: Math.max.apply(null, allUsers.map(function(u){return u.id;})) + 1, name:name, email:email, role:role, isActive:true });
            } else {
                var u = allUsers.find(function(x){return x.id === editingUserId;});
                if (u) { u.name = name; u.role = role; }
            }
            renderUserTable(allUsers); closeUserModal();
            showToast('[DEMO] User saved locally','success');
            setBtnLoading(btn, false);
        });
}

function toggleUserStatus(id) {
    fetch(API_BASE + '/users/' + id + '/status/toggle', { method:'PUT', credentials:'include' })
        .then(function(r){return r.json();})
        .then(function(data){ if(data.success){showToast(data.message,'success');loadUsers();}else{showToast(data.message||'Failed','fail');} })
        .catch(function(){
            var u = allUsers.find(function(x){return x.id === id;});
            if (u) { u.isActive = !u.isActive; renderUserTable(allUsers); }
            showToast('[DEMO] Status toggled locally','success');
        });
}

function deleteUser(id) {
    openConfirm('Permanently delete this user?', function(){
        fetch(API_BASE + '/users/' + id, { method:'DELETE', credentials:'include' })
            .then(function(r){return r.json();})
            .then(function(data){ if(data.success){showToast(data.message,'success');loadUsers();} })
            .catch(function(){ allUsers = allUsers.filter(function(u){return u.id !== id;}); renderUserTable(allUsers); showToast('[DEMO] Deleted locally','success'); });
    });
}
...
function loadOrders() {
    ...
    tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;color:rgba(255,255,255,.4);padding:3rem;">Loading orders...</td></tr>';
    // Load summary for stats
    fetch(API_BASE + '/orders/summary', { credentials:'include' })
        .then(function(r){return r.ok ? r.json() : null;}).then(function(sum){ if(sum) updateOrderStats(sum); }).catch(function(){}).finally(function(){
        fetch(API_BASE + '/orders', { credentials:'include' })
            .then(function(r){return r.json();})
            .then(function(data){ allOrders = data; renderOrderTable(allOrders); })
            .catch(function(){
                allOrders = [
                    { id:101, supplierName:'TechSupply Co.',   orderedBy:1, orderDate:'2025-05-01', expectedDate:'2025-05-08', status:'RECEIVED',  notes:'Standard delivery' },
                    { id:102, supplierName:'OfficeWorld Ltd.', orderedBy:2, orderDate:'2025-05-03', expectedDate:'2025-05-10', status:'APPROVED',  notes:'Urgent order' },
                    { id:103, supplierName:'FastPack Inc.',    orderedBy:1, orderDate:'2025-05-05', expectedDate:'2025-05-12', status:'PENDING',   notes:'' },
                    { id:104, supplierName:'TechSupply Co.',   orderedBy:2, orderDate:'2025-05-06', expectedDate:'2025-05-15', status:'PENDING',   notes:'Bulk order' },
                    { id:105, supplierName:'OfficeWorld Ltd.', orderedBy:1, orderDate:'2025-05-08', expectedDate:'2025-05-14', status:'CANCELLED', notes:'Payment issue' }
                ];
                renderOrderTable(allOrders);
                showToast('[DEMO] Orders API offline','fail');
            });
    });
}

function updateOrderStats(sum) {
    var stats = document.getElementById('tab-order').querySelectorAll('.stat-value');
    if (stats.length >= 4) {
        stats[0].textContent = sum.total || 0;
        stats[1].textContent = sum.pending || 0;
        stats[2].textContent = sum.approved || 0;
        stats[3].textContent = sum.received || 0;
    }
}
...
function renderOrderTable(orders) {
    var tbody = document.getElementById('order-table-body');
    tbody.innerHTML = '';
    if (!orders.length) { tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:3rem;color:rgba(255,255,255,.4);">No orders found.</td></tr>'; return; }
    orders.forEach(function(o){
        var stCls = o.status === 'RECEIVED' ? 'delivered' : o.status === 'APPROVED' ? 'active' : o.status === 'CANCELLED' ? 'suspended' : 'pending';

        // ✅ FIX: Look up supplier name from allSuppliers if only supplierId is returned
        var supplierDisplay = o.supplierName;
        if (!supplierDisplay && o.supplierId && allSuppliers && allSuppliers.length) {
            var sup = allSuppliers.find(function(s){ return s.id === o.supplierId; });
            supplierDisplay = sup ? sup.name : ('#' + o.supplierId);
        }
        supplierDisplay = supplierDisplay || 'Unknown';

        var tr = document.createElement('tr');
        tr.innerHTML = '<td style="font-family:Space Mono,monospace;color:var(--neon);">#' + o.id + '</td>' +
            '<td style="color:#fff;">' + escHtml(supplierDisplay) + '</td>' +
            '<td>' + (o.orderedBy || '\u2014') + '</td>' +
            '<td>' + fmtDate(o.orderDate) + '</td>' +
            '<td>' + fmtDate(o.expectedDate) + '</td>' +
            '<td><span class="badge-status ' + stCls + '">' + o.status + '</span></td>' +
            '<td>' + escHtml(o.notes || '\u2014') + '</td>' +
            '<td><div class="action-btns">' +
            '<button class="btn-edit" onclick="openOrderStatusModal(' + o.id + ')">Status</button>' +
...
function openOrderStatusModal(orderId) {
    ...
    sub.textContent   = '// ORDER #' + o.id;
    body.innerHTML = '<div class="modal-field"><label class="modal-label">Current Status</label><span class="badge-status ' + (o.status==='RECEIVED'?'delivered':o.status==='APPROVED'?'active':o.status==='CANCELLED'?'suspended':'pending') + '">' + o.status + '</span></div>' +
        '<div class="modal-field"><label class="modal-label">New Status</label>' +
        '<select class="modal-select" id="ord-status-new">' +
        ...
        '</select></div>' +
        '<input type="hidden" id="ord-status-id" value="' + o.id + '">';
    window._genericSaveTarget = 'order';
    overlay.classList.add('open');
}
...
function openOrderModal() {
    ...
    var currentUserId = sessionStorage.getItem('userId') || '1';

    title.textContent = 'New Purchase Order';
    sub.textContent   = '// CREATE SUPPLIER ORDER';
    body.innerHTML = '<div class="modal-row">' +
        '<div class="modal-field"><label class="modal-label">Supplier *</label><select class="modal-select" id="ord-supplier">' + supOpts + '</select></div>' +
        '<div class="modal-field"><label class="modal-label">Ordered By (User ID)</label><input type="number" class="modal-input" id="ord-by" placeholder="1" value="' + currentUserId + '"></div>' +
        '</div>' +
        '<div class="modal-row">' +
        '<div class="modal-field"><label class="modal-label">Status</label><select class="modal-select" id="ord-status"><option value="PENDING" selected>PENDING</option><option value="APPROVED">APPROVED</option><option value="RECEIVED">RECEIVED</option><option value="CANCELLED">CANCELLED</option></select></div>' +
        '<div class="modal-field"><label class="modal-label">Expected Date</label><input type="date" class="modal-input" id="ord-expected" value=""></div>' +
        '</div>' +
        '<div class="modal-field"><label class="modal-label">Notes</label><textarea class="modal-input" id="ord-notes" placeholder="Order notes..." style="min-height:60px;resize:vertical;"></textarea></div>';
    document.getElementById('ord-expected').value = today;
    window._genericSaveTarget = 'order';
    overlay.classList.add('open');
}
...
function saveOrder() {
    // Check if this is a status update or a new order
    var statusId = document.getElementById('ord-status-id');
    if (statusId) {
        // Status update
        var id = parseInt(statusId.value);
        var newStatus = document.getElementById('ord-status-new').value;
        fetch(API_BASE + '/orders/' + id + '/status', { method:'PUT', credentials:'include', headers:{'Content-Type':'application/json'}, body:JSON.stringify({status:newStatus}) })
            .then(function(r){return r.json();})
            .then(function(data){ if(data.success){showToast(data.message,'success');closeGenericModal();loadOrders();}else{showToast(data.message||'Failed','fail');} })
            .catch(function(){
                var o = allOrders.find(function(x){return x.id === id;});
                if (o) o.status = newStatus;
                renderOrderTable(allOrders); closeGenericModal();
                showToast('[DEMO] Status updated locally','success');
            });
        return;
    }
    ...
    var orderedBy = parseInt(document.getElementById('ord-by').value) || null;
    var status = document.getElementById('ord-status').value;
    var expectedDate = document.getElementById('ord-expected').value;
    var notes = document.getElementById('ord-notes').value.trim();
    if (!supplierId) { showToast('Please select a supplier.','fail'); return; }

    var body = { supplierId: parseInt(supplierId), status: status, notes: notes };
    if (orderedBy) body.orderedBy = orderedBy;
    if (expectedDate) body.expectedDate = expectedDate;

    fetch(API_BASE + '/orders', { method:'POST', credentials:'include', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body) })
        .then(function(r){return r.json();})
        .then(function(data){ if(data.success){showToast(data.message,'success');closeGenericModal();loadOrders();}else{showToast(data.message||'Failed','fail');} })
        .catch(function(){
            var newId = allOrders.length ? Math.max.apply(null, allOrders.map(function(o){return o.id;})) + 1 : 1;
            var sup = allSuppliers.find(function(s){return String(s.id) === supplierId;});
            allOrders.push({ id:newId, supplierName: sup ? sup.name : 'Unknown', orderedBy: orderedBy || 1, orderDate: new Date().toISOString().split('T')[0], expectedDate: expectedDate, status: status, notes: notes });
            renderOrderTable(allOrders); closeGenericModal();
            showToast('[DEMO] Order created locally','success');
        });
}

function deleteOrder(id) {
    openConfirm('Delete this order?', function(){
        fetch(API_BASE + '/orders/' + id, { method:'DELETE', credentials:'include' })
            .then(function(r){return r.json();})
            .then(function(data){ if(data.success){showToast(data.message,'success');loadOrders();} })
            .catch(function(){ allOrders = allOrders.filter(function(o){return o.id !== id;}); renderOrderTable(allOrders); showToast('[DEMO] Deleted locally','success'); });
    });
}
